	...
	if self:GetPos():Distance(LocalPlayer():GetPos()) < 1000 then
		...
		cam.Start3D2D(self:GetPos()+self:GetUp()*80, Ang, 0.05)
			render.PushFilterMin(TEXFILTER.ANISOTROPIC)
				draw.SimpleTextOutlined( 'Переработка Мусора', "font_base_large", -3, 0, Color(173,236,168), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, Color(0, 0, 0, 255))
				draw.SimpleTextOutlined( 'Только для Авторизированных Работников ГСР', "font_base_54", -3, 100, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, Color(0, 0, 0, 255))
			render.PopFilterMin()
		cam.End3D2D()
	end
end
...
net.Receive('TrashTerminal_OpenMenu', function()
	...
	-- 	netstream.Start("TrashTerminal_SaleAllItems", {npc = target});
	-- 	Menu:Close()
	-- end
    ...
    for k, item in pairs(items) do
        ...
        btn_items[k].DoClick = function()
            ...
            local option = menu:AddOption( "Продать за "..rp.FormatMoney(rp.cfg.Trash[item.Class]*1.6), function()
                -- netstream.Start("TrashTerminal_SaleItem", {npc = target, item_id = item.ID});
				net.Start('TrashTerminal_SaleItem')
