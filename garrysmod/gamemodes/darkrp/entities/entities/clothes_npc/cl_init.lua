	...
	if self:GetPos():Distance(LocalPlayer():GetPos()) < 1000 then
		...
		cam.Start3D2D(self:GetPos()+self:GetUp()*80, Ang, 0.05)
			render.PushFilterMin(TEXFILTER.ANISOTROPIC)
				draw.SimpleTextOutlined( 'Продавец Одежды', "font_base_large", -3, 0, Color(173,236,168), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, Color(0, 0, 0, 255))
				draw.SimpleTextOutlined( '', "font_base_54", -3, 100, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, Color(0, 0, 0, 255))
			render.PopFilterMin()
		cam.End3D2D()
	end
end
...
net.Receive('NPCClothes_OpenMenu', function()
    ...
    panel.Paint = function( self, w, h )
        ...
        draw.SimpleText('Одежда', "font_base_24", w*.5, 10, Color( 240, 240, 240, 255 ), 1)
	...
	-- 	netstream.Start("NPCClothes_SaleAllItems", {npc = target});
	-- 	Menu:Close()
	-- end
    ...
    local btn_items = {}
    local k = 0
    for name, _ in pairs(rp.clothes) do
        k = k + 1
        ...
        btn_items[k].DoClick = function()
            ...
            local option = menu:AddOption( "Продать за "..rp.FormatMoney(rp.cfg.Trash[item.Class]), function()
                -- netstream.Start("NPCClothes_SaleItem", {npc = target, item_id = item.ID});
				-- net.Start('NPCClothes_SaleItem')
        ...
        btn_items[k].Paint = function( self, w, h )
            ...
            draw.SimpleText(item.name, "font_notify", 24+3, 3, Color( 240, 240, 240, 255 ), 0)
        ...
        local icon = vgui.Create('SpawnIcon', btn_items[k])
        icon:SetPos(0,0)
        icon:SetSize(24,24)
