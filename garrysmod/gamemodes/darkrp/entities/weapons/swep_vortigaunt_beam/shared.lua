...
function SWEP:ShootEffect(EFFECTSTR,startpos,endpos)
	...
	if CLIENT then view=GetViewEntity() else view=pPlayer:GetViewEntity() end
		if ( !pPlayer:IsNPC() && view:IsPlayer() ) then
            ...
            if att then
			    util.ParticleTracerEx( EFFECTSTR, self.Weapon:GetAttachment( att ).Pos,endpos, true, pPlayer:GetViewModel():EntIndex(), att );
            end
        else
            local att = pPlayer:LookupAttachment( "anim_attachment_rh" )
            if att then
			    util.ParticleTracerEx( EFFECTSTR, pPlayer:GetAttachment( att ).Pos,endpos, true,pPlayer:EntIndex(), att );
            end
        end
end
