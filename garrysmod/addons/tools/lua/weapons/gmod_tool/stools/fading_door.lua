...
local function dooEet(ply, ent, stuff)
	if ent:GetNWBool("isFadingDoor") then
		...
		if ent:GetNWInt("PropHealth") <= 0 then
			ent:SetNWInt("PropHealth", 1500)
		end
...
if SERVER then
	hook.Add( "EntityTakeDamage", "fading_door.EntityTakeDamage", function(ent, dmg)
		if ent:GetNWBool("IsFadingDoor") and ent:GetNWInt("PropHealth") then
			ent:SetNWInt("PropHealth", ent:GetNWInt("PropHealth") - dmg:GetDamage())
			if ent:GetNWInt("PropHealth") <= 0 then
				ent:Remove()
			end
		end
