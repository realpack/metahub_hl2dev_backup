...
function PANEL:SetTitle(sText)
	self.title = self.title or ui.Create("DLabel",self,function(t)
		t:Dock(TOP)
		t:SetTall(20)
		t:SetFont("ui.20")
...
function PANEL:AddColumn(sName,iWidePx)
	self.columns_panel = self.columns_panel or ui.Create("Panel",self,function(p)
		p:SetTall(15)

		self.header_tall = self.header_tall + p:GetTall()
	end)

	table.insert(self.columns,ui.Create("Panel",self.columns_panel,function(clmn)
...
function PANEL:AddLine(...)
	...
	local iKek = table.insert(self.lines,
		self.scroll:AddItem(ui.Create("Panel",function(line)
			line:SetTall(18)
			line:Dock(TOP)
			line.columns = {}
			...
			for i,val in ipairs(rows) do
				line.columns[i] = ui.Create("DButton",line,function(row) -- Было Panel
					row:SetText("")
					...
					row.Paint = function(s,w,h)
						...
						surface.SetTextPos((w - surface.GetTextSize(val)) / 2,0)
...
vgui.Register("ui_table", PANEL, "Panel")
-- IGS.UI()