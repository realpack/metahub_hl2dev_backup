...
function PANEL:SetName(sName)
	...
	self.name = self.name or ui.Create("DLabel",self,function(lbl)
		lbl:SetTall(20)
...
function PANEL:SetSign(sSignature)
	self.sign = self.sign or ui.Create("DLabel",self,function(lbl)
		lbl:SetTall(15)
...
function PANEL:SetBottomText(sBottomText)
	self.bottom = self.bottom or ui.Create("DLabel",self,function(lbl)
		lbl:SetTall(15)
		lbl:SetFont("roboto_15")
		lbl:SetTextColor(IGS.col.TEXT_SOFT)
...
function PANEL:SetIcon(sIco,bIsModel) -- :SetIcon() для сброса
	...
	if !self.icon then
		local icobg = ui.Create("Panel",self)
		icobg:SetSize(40,40)
		icobg:SetPos(2,2)
		icobg.Paint = IGS.S.RoundedPanel

		self.icon = bIsModel and ui.Create("DModelPanel",icobg,function(mdl)
			mdl:Dock(FILL)
		...
		ui.Create("wmat_ui",icobg,function(ico)
			ico:Dock(FILL)
