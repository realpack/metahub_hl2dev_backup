...
local curTime = CurTime();
	if (!self.nextUse or curTime >= self.nextUse) then
		if !EML_Meth_UseSalesman then
			...
			activator:SendLua("local tab = {Color(255,255,255), [[Вы подобрали ]], Color(1,241,249,255), [["..math.Round(self:GetNWInt("amount")).."]], Color(255, 255, 255), [[ фунтов Кристального Мета.]] } chat.AddText(unpack(tab))");
			self:VisualEffect();
			self.nextUse = curTime + 0.5;		
		end;
	end;
