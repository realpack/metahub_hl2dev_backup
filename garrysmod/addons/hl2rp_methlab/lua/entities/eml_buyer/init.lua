...
function ENT:AcceptInput(name, activator, caller)	
	if (!self.nextUse or CurTime() >= self.nextUse) then
		if (name == "Use" and caller:IsPlayer() and (caller:GetNWInt("player_meth") == 0)) then
			...
			timer.Simple(0.25, function() self:EmitSound(table.Random(EML_Meth_Salesman_NoMeth_Sound), EML_Sound_Level, EML_Sound_Pitch, EML_Sound_Volume) end);
		elseif (name == "Use") and (caller:IsPlayer()) and (caller:GetNWInt("player_meth") > 0) then
			caller:addMoney(caller:GetNWInt("player_meth"));
			caller:SendLua("local tab = {Color(1,241,249,255), [[Скупщик: ]], Color(255,255,255), [["..table.Random(EML_Meth_Salesman_GotMeth)..", возми за свою работу ]], Color(128, 255, 128), [["..caller:GetNWInt("player_meth").."K.]] } chat.AddText(unpack(tab))");
			caller:SetNWInt("player_meth", 0);
			if (EML_Meth_MakeWanted) then
				caller:wanted(nil, "Продажа Мета");
			end;
			timer.Simple(0.25, function() self:EmitSound(table.Random(EML_Meth_Salesman_GotMeth_Sound), EML_Sound_Level, EML_Sound_Pitch, EML_Sound_Volume) end);
			timer.Simple(2.5, function() self:EmitSound("vo/npc/male01/moan0"..math.random(1, 5)..".wav", EML_Sound_Level, EML_Sound_Pitch, EML_Sound_Volume) end);
		end;
