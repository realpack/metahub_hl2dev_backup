...
timer.Simple(1,
	function()
		...
		for k, v in pairs(file.Find("jb/"..string.lower(game.GetMap()).."/*.txt", "DATA")) do
			local treePosFile = file.Read("jb/"..string.lower(game.GetMap()).."/"..v, "DATA");
			 
			local spawnNumber = string.Explode(" ", treePosFile);         
				   
			local tree = ents.Create("swm_wood");
	...
	function spawnTreePos(ply, cmd, args)
		if (ply:IsAdmin() or ply:IsSuperAdmin()) then
			...
			if file.Exists( "jb/"..string.lower(game.GetMap()).."/swm_wood_".. fileTreeName ..".txt", "DATA") then
				ply:SendLua("local tab = {Color(0,255,0,255), [[Sawmill - ]], Color(255,255,255), [[This uniqueID is already in use, choose another one or type 'swm_tree_remove "..fileTreeName.."' in console to remove this one.]] } chat.AddText(unpack(tab))");
				return;
			end;
			...
			file.Write("jb/"..string.lower(game.GetMap()).."/swm_wood_".. fileTreeName ..".txt", ""..(treeVector[1]).." "..(treeVector[2]).." "..(treeVector[3]).." "..(treeAngles[1]).." "..(treeAngles[2]).." "..(treeAngles[3]).."", "DATA");
			ply:SendLua("local tab = {Color(0,255,0,255), [[Sawmill - ]], Color(255,255,255), [[New pos for the tree has been set.]] } chat.AddText(unpack(tab))");
		else
	...
	concommand.Add("swm_tree_spawn", spawnTreePos);
	...
	function removeTreePos(ply, cmd, args)
		if (ply:IsAdmin() or ply:IsSuperAdmin()) then
			...
			if file.Exists("jb/"..string.lower(game.GetMap()).."/swm_wood_"..fileTreeName..".txt", "DATA") then
				file.Delete("jb/"..string.lower(game.GetMap()).."/swm_wood_"..fileTreeName..".txt");
				ply:SendLua("local tab = {Color(0,255,0,255), [[Sawmill - ]], Color(255,255,255), [[This tree has been removed. Restart your server!]] } chat.AddText(unpack(tab))");
				return;
			end;
	...
	concommand.Add("swm_tree_remove", removeTreePos);
end