...
function ENT:Initialize()
	self:SetModel(table.Random(SWM_TREE_RED_MODELS))
	...
	self.Replace = false
	phys:Wake()
	self:SetNWInt("health", SWM_TREE_RED_HEALTH)
...
function ENT:Think()
	if (!self.Replace) and (self:GetNWInt("health") <= 0)  then
		self.Replace = true
		self.ReplaceTime = CurTime() + SWM_TREE_RED_REPLACE_TIMER
		...
		log2:Spawn()
		local log3 = ents.Create('swm_log')
		log3:SetPos(self:GetPos()+Vector(0,0,144))
		log3:Spawn()
		local log4 = ents.Create('swm_log')
		log4:SetPos(self:GetPos()+Vector(0,0,201))
		log4:Spawn()
		constraint.Weld(log, log2, 0, 0, 500, true, false)
		constraint.Weld(log2, log3, 0, 0, 500, true, false)
		constraint.Weld(log3, log4, 0, 0, 500, true, false)
