...
function ENT:Draw()
	...
	local text = "Прочность ящика: "..math.Round((100/SWM_TREE_RED_HEALTH)*self:GetNWInt("health")).."%"
	local TextWidth = surface.GetTextSize(text)
	local width = ((TextWidth+10)/100)*((100/SWM_TREE_RED_HEALTH)*self:GetNWInt("health"))
	...
	if LocalPlayer():GetPos():Distance(self:GetPos()) < self:GetNWInt("distance") then
		cam.Start3D2D(Pos+Ang:Up()*20+Ang:Right()*-30, Ang, 0.13)
			draw.RoundedBox( 0, -TextWidth*0.5 , -1, width, 37, Color(225,165,0,170) )
